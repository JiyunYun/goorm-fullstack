...
public class CommentRepositoryCustom {
  ...
  public Comment find(Long commentId) {
    return queryFactory.select(QComment.comment)
      .from(QComment.comment)
      .where(QComment.comment.commentNo.eq(commentId))
      .fetchOne();
  }
  ...
  public Comment find(Long commentId, Long boardId, CommentStatus commentStatus) {
    return queryFactory.select(QComment.comment)
      .from(QComment.comment)
      .where(QComment.comment.commentNo.eq(commentId))
      .where(QComment.comment.commentStatus.eq(commentStatus))
      .where(QBoard.board.boardNo.eq(boardId))
      .fetchOne();
  }
}
